import {
  Body,
  Button,
  Container,
  Head,
  Heading,
  Html,
  Img,
  Link,
  Preview,
  Section,
  Text,
  Font,
  Tailwind,
} from "react-email";

interface Props {
  firstName: string;
  verificationUrl: string;
}

export const SubscriptionVerificationEmail = ({ 
  firstName = "there",
  verificationUrl = "https://www.sakhiledumisa.com"
}: Props) => {
  return (
    <Html lang="en" dir="ltr">
      <Head>
        <Font
          fontFamily="Space Grotesk"
          fallbackFontFamily="Arial"
          webFont={{
            url: "https://mail.sakhiledumisa.com/space-grotesk.woff2",
            format: "woff2",
          }}
          fontWeight={400}
          fontStyle="normal"
        />
      </Head>
      <Preview>Please confirm your subscription to Sakhile's newsletter.</Preview>
      <Tailwind
        config={{
          theme: {
            extend: {
              fontFamily: {
                sans: ["'Space Grotesk'", "sans-serif"],
              },
            },
          },
        }}
      >
        <Body className="bg-white my-auto mx-auto font-sans">
          <Container className="max-w-[600px] mx-auto p-[40px_20px]">
            {/* Header / Brand Identity */}
            <Section className="mb-[32px] text-left">
              <Link href="https://www.sakhiledumisa.com">
                <Img
                  src="https://mail.sakhiledumisa.com/logo.png"
                  width="52"
                  height="52"
                  alt="Logo"
                  className="rounded-lg inline-block"
                />
              </Link>
            </Section>

            {/* Core Content Engine */}
            <Section>
              <Heading className="text-black text-base font-semibold leading-tight mb-4 text-left">
                Confirm your subscription
              </Heading>

              <Text className="text-[#444444] text-sm leading-relaxed mb-6 text-left font-normal">
                Hello {firstName}. You're receiving this because you requested to join my newsletter 
                list at sakhiledumisa.com. To prevent spam and activate your subscription, 
                please click the verification link below.
              </Text>

              {/* Primary Call to Action Button */}
              <Section className="text-left mt-2 mb-8">
                <Button
                  href={verificationUrl}
                  className="bg-black text-white text-xs font-medium px-5 py-3 rounded-md no-underline inline-block"
                >
                  Verify Subscription →
                </Button>
              </Section>

              {/* Explicit Compliance & Control Footer */}
              <Section className="border-t border-solid border-[#eeeeee] pt-6 mt-8">
                <Text className="text-[#868686] text-xs leading-relaxed m-0 text-left">
                  If you did not request this subscription, you can safely ignore this email. 
                  No further actions will be taken, and your address will not be added to any broadcast list.
                </Text>
              </Section>
            </Section>
          </Container>
        </Body>
      </Tailwind>
    </Html>
  );
};

export default SubscriptionVerificationEmail;